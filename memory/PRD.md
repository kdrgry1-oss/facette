...
## Iteration 76 (2026-05-21) — Trendyol Kâr Oranı SSOT Fix 💰

### 🐛 Şikayet
"Ayarlar sayfasında %20 ama ürün modalında %15 yazıyor."

### 🔍 Root Cause
İki ayrı DB kaydı:
- `settings.id="trendyol"` → `default_markup` (Trendyol Integration kartı)
- `settings.id="main"` → `trendyol_markup` (Ana Ayarlar sayfası)

Backend ana ayarlardaki değeri sadece "0" durumunda fallback olarak kullanıyordu → kullanıcı %20'yi main'e yazdı, trendyol settings'te eski 15 kaldı → ürün modal'ı 15 gösteriyordu.

### 🔧 Fix (`backend/routes/integrations.py`)
- `get_trendyol_config()` ve `get_trendyol_settings`: `main.trendyol_markup` ÖNCELİKLİ. Eğer set edilmişse her zaman onu kullan.
- `save_trendyol_settings` POST: Trendyol kartından markup değiştirildiğinde otomatik olarak `main.trendyol_markup`'e de yazılıyor (SSOT).
- DB senkron: mevcut `trendyol.default_markup` 15 → 20 olarak güncellendi.

### ✅ Test
- `GET /api/integrations/trendyol/settings` → `default_markup: 20.0`
...
```
https://marketplace-sync-hub-5.preview.emergentagent.com/api/orders/cargo/mng-webhook
```
...
- MNG'ye webhook URL bildirimi: `https://marketplace-sync-hub-5.preview.emergentagent.com/api/orders/cargo/mng-webhook`
