/**
 * IYSManagement.jsx — İYS (İleti Yönetim Sistemi) Yönetimi
 *
 * İYS hizmeti Netgsm İş Ortağı API'si üzerinden kullanılır.
 * Netgsm hesap bilgileri Ayarlar → Bildirimler → Netgsm'den alınır;
 * bu sayfadan yalnızca İYS Marka Kodu tanımlanır.
 *
 * Backend: /api/admin/iys/{status,settings,test-connection,query,query-batch,register,register-batch}
 */
...
export default function IYSManagement() {
  ...
  const saveBrand = async () => {
    ...
    setSavingBrand(true);
    try {
      await axios.post(`${API}/admin/iys/settings`, { brand_code: brandCode.trim() }, auth);
      toast.success("İYS Marka Kodu kaydedildi");
      loadStatus();
    } catch (e) {
  ...
  const runRegister = async () => {
    ...
    try {
      ...
      r.data.ok
        ? toast.success(r.data.message || "İzin kuyruğa alındı")
        : toast.error(r.data.message || "İzin eklenemedi");
    } catch (e) {
      toast.error(e.response?.data?.detail || e.message);
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Netgsm İş Ortağı API'si üzerinden izin sorgulama ve yükleme. Ticari SMS/e-posta öncesi ONAY kontrolü yasal zorunluluktur.
          </p>
        </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className={cardCls}>
          ...
          {status ? (
            <div className="space-y-2 text-sm">
              ...
              <div className="text-gray-600">Netgsm kullanıcı: {status.username_set ? "✓ tanımlı" : "✗ eksik"} · Şifre: {status.password_set ? "✓ tanımlı" : "✗ eksik"}</div>
        ...
        <div className={cardCls}>
          <h2 className="font-medium mb-3 flex items-center gap-2"><Save size={16} /> İYS Marka Kodu</h2>
          <p className="text-xs text-gray-500 mb-3">
            İYS panelinizde markanıza atanan koddur. Netgsm portalında <b>SMS Hizmeti → NetİYS</b> altında da görüntülenebilir.
            Netgsm kullanıcı adı/şifresi <b>Ayarlar → Bildirimler → Netgsm</b> bölümünden alınır.
          </p>
          <div className="flex gap-2">
            <input value={brandCode} onChange={e => setBrandCode(e.target.value)}
              placeholder="Örn: 123456" className={inputCls} />
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div className={cardCls}>
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs text-gray-400">İzinler Netgsm kuyruğuna alınır; nihai sonuç birkaç dakika içinde İYS'de işlenir.</p>
          </div>
        </div>
      </div>
