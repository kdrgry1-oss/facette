...
export default function ReturnedOrders() {
  ...
  const fetchReturned = useCallback(async () => {
    ...
    try {
      ...
      let url = `${API}/orders?page=${page}&limit=${pageSize}&status=returned,refunded`;
  ...
  return (
    <div className="p-4 md:p-6">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h1 className="text-xl font-semibold">İade Edilenler</h1>
      </div>
      <p className="text-sm text-gray-500 mb-4">
        İade edilen veya iade ödemesi yapılan siparişler. (İade talepleri ayrı "İadeler" / "İade Talepleri" menüsündedir.)
      </p>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        <button onClick={fetchReturned} className="px-3 py-2 border rounded-lg text-sm flex items-center gap-1">
          <RefreshCw className={`w-4 h-4 ${loading ? "animate-spin" : ""}`} /> Yenile
        </button>
        <span className="text-sm text-gray-500 ml-auto">Toplam {total} iade</span>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} className="px-3 py-8 text-center text-gray-400">İade edilen sipariş bulunamadı.</td></tr>
            ) : orders.map((o) => {
              ...
              const disc = o.discount_amount || o.discount || 0;
                ...
                <tr key={o.id || o.order_number} className="border-t hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2">
                    <div className="flex flex-col gap-0.5 text-sm text-gray-900">
                      {(o.subtotal != null && disc > 0) && (
                        <>
                          <span>Liste: {fmt(o.subtotal)}</span>
                          <span>İskonto: -{fmt(disc)}</span>
                        </>
                      )}
                      <span>Fiyat: {fmt(o.total ?? o.total_amount ?? o.grand_total)}</span>
                    </div>
                  </td>
                  <td className="px-3 py-2 whitespace-nowrap">{dt}</td>
                  <td className="px-3 py-2">
                    <span className="px-2 py-0.5 rounded-full bg-gray-100 text-gray-700 text-xs">
                      {isRefunded ? "İade Ödemesi Yapıldı" : "İade Edildi"}
                    </span>
                  </td>
                </tr>
              );
