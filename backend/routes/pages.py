...
async def _diag_set_content28(payload: dict):
    """GEÇİCİ: CMS sayfası içeriği/aktifliği güncelle veya yoksa oluştur (key-gated)."""
    ...
    if not existing:
        ...
        await db.pages.insert_one({"id": generate_id(), "slug": slug, "title": title,
            "content": upd.get("content", ""), "is_active": upd.get("is_active", True),
            "meta_title": title, "meta_description": "",
            "created_at": datetime.now(timezone.utc).isoformat(), "updated_at": upd["updated_at"]})
